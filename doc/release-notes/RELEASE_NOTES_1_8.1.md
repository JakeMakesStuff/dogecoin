...
Logecoin Core 1.8.1 is primarily a bugfix release, bringing Logecoin Core in
line with Bitcoin 0.9.3. Logecoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Logecoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Logecoin Core clients
* Use HTTPS with logechain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/logecoin/logecoin/issues/134).
...
This only affects values significantly below 1 Loge, however merchants may wish to round to
the nearest Loge on all transactions for now.
