...
*Setup instructions for a Gitian build of Logecoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Logecoin
...
- [Building Logecoin Core](#building-logecoin-core)
...
Clone the git repositories for logecoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/logecoin/logecoin.git
git clone https://github.com/logecoin/gitian.sigs.git
```
...
in the logecoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Logecoin Core
----------------

To build Logecoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the logecoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/logecoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/logecoin/logecoin
...
For example:
```bash
URL=https://github.com/laanwj/logecoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit logecoin=${COMMIT} --url logecoin=${URL} ../logecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit logecoin=${COMMIT} --url logecoin=${URL} ../logecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit logecoin=${COMMIT} --url logecoin=${URL} ../logecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the logecoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../logecoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/logecoin/logecoin-detached-sigs.git

BTCPATH=/some/root/path/logecoin
SIGPATH=/some/root/path/logecoin-detached-sigs

./bin/gbuild --url logecoin=${BTCPATH},signature=${SIGPATH} ../logecoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/logecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/logecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/logecoin-osx-build.assert
```
...
[logecoin/gitian.sigs](https://github.com/logecoin/gitian.sigs/) repository, or if that's not possible create a pull
