...
#if defined(NDEBUG)
# error "Logecoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Logecoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetLogecoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Logecoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("logecoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Logecoin: BIP30 has been active since inception
    ...
    // Logecoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetLogecoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Logecoin: Disallow AuxPow blocks before it is activated.
    ...
    // Logecoin: Version 2 enforcement was never used
    ...
    // Logecoin: Introduce supermajority rules for v4 blocks
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Logecoin: We probably want to disable this
    int nLockTimeFlags = 0;
